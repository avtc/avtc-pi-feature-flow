...
/** Set up the extension and return the /ff:next handler + helpers. */
function setup() {
  const fake = createFakePi();
  workflowMonitorExtension(fake.api as unknown as ExtensionAPI);

  const workflowNextRaw = fake.registeredCommands.get("ff:next");
  if (!workflowNextRaw) throw new Error("ff:next command not registered");
...
describe("ff:next (rewritten)", () => {
  ...
  test("advances from design even when no design doc is recorded (pointer model)", async () => {
    ...
    // In the pointer model ff:next always routes the current phase forward (the
    // design-doc precondition was removed); it advances to plan.
    const planMsg = fake.sentMessages.find((m) => m.message.includes("ff-plan"));
  ...
  test("skips review and proceeds to finish when maxFeatureReviewRounds=off + uatMode=after-finish", async () => {
    ...
    const reviewMsg = fake.sentMessages.find((m) => m.message.includes('name="ff-review"'));
  ...
  test("ff:next review→UAT handoff omits the worth-notes pointer when notes are absent", async () => {
    setSetting("uatMode", "after-review");
    const { fake, workflowNext } = setup();
    const { ctx, notifications } = makeCtx();
